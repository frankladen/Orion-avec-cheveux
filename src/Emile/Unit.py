...
class Unit(PlayerObject):
    ...
    NAME = ('Unité','Scout', "Vaisseau d'attaque", "Vaisseau de Transport", "Cargo", 'Unité terrestre', 'Unité de collecte', 'Unité spéciale', 'Unité d\'attaque', 'Unité de construction')
    MINERAL=0
    GAS=1
    FOOD=2
    SIZE=((0,0),  (18,15), (28,32), (32,29), (20,30),(24,24),(23,37), (32,32), (36,33),(20,20))
        ...
        if isinstance(ressource, w.NuclearSite):
            ...
            if self.position[0] < ressource.position[0] or self.position[0] > ressource.position[0]:
                if self.position[1] < ressource.position[1] or self.position[1] > ressource.position[1]:
                    ...
                    self.move()
            if arrived:
                player.ressources[player.NUCLEAR] += self.container
                self.container = 0
                self.flag.finalTarget = self.position
                self.flag.flagState = FlagState.STANDBY
