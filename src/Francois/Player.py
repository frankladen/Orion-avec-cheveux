...
class Player():
    ...
    def __init__(self, name, game, id , colorId):
        ...
        self.ressources = [10000,10000000,10]
        self.isAlive = True
    ...
    def killUnit(self, killedIndexes):
        self.units[killedIndexes].kill()
        if killedIndexes == self.id:
            if self.units[killedIndexes] in self.selectedObjects:
                self.selectedObjects.remove(self.units[killedIndexes])
