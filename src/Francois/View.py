...
class View():              
    ...
    def fLobby(self):
        ...
        if self.parent.server != None:
            ...
            c = Checkbutton(lobbyFrame, text = 'Prêt à jouer ?',variable = self.ready, offvalue = -1, command=self.parent.readyToPlay, bg="white", fg="white")
            c.grid(row=(pNum+3),column=1)
            Label(lobbyFrame, text='Joueur prêt à jouer : %s/%s' % (self.parent.server.getPlayersReady(), len(self.parent.server.getSockets())), fg="white", bg="black").grid(row=(pNum+1), column=1)
        return lobbyFrame
