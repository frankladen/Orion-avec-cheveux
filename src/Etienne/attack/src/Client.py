...
class Controller():
    ...
    def startGame(self):
        ...
        self.players.append(p.Player("Bobby", 1))
    ...
    def doAction(self, changeString):
        ...
        if action == str(FlagState.MOVE) or action == str(FlagState.STANDBY):
            ...
            print("on a pull un changement qui veut qu'on fasse attaquer")
            ...
            #ici, le target sera l'index de l'unité dans le tableau de unit du player ciblé
        elif action == 'deleteAllUnits':
            self.players[actionPlayerId].units = []
        elif action == 'addunit':
            if unitIndex[0] == 'Scout':
            ...
            self.players[actionPlayerId].units.pop(int(unitIndex[0]))
