...
class Controller():
 ...
 # Pour changer le flag des unités selectionne pour la construction        
    def setBuildingFlag(self,x,y):
        ...
        self.pushChange(units, Flag(i,t.Target([x,y,0]),FlagState.BUILD))
        ...
        if action == str(FlagState.MOVE) or action == str(FlagState.STANDBY) or action == str(FlagState.PATROL):
            ...
            for i in unitIndex:
                if i != '':
                    ...
                    wp = Waypoint('Waypoint', Building.WAYPOINT, [target[0],target[1]], actionPlayerId)
                    self.players[actionPlayerId].buildings.append(wp)
                    self.players[actionPlayerId].units[int(i)].changeFlag(wp,int(action))
