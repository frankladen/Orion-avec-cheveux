...
class View():              
    def __init__(self, parent):
        ...
        self.building = PhotoImage(file='images\\Building\\geek.gif')
        self.attacking = False
        self.building = False
    ...
    def drawBuilding(self, building,  player, isInFOW):
        ...
        if self.parent.players[self.parent.playerId].camera.isInFOV(buildingPosition):
            ...
            print('milieu')
            if not isInFOW:
                self.gameArea.create_image(distance[0]+1, distance[1], image=self.building)
                print('fin')
    ...
    def leftclic(self, eve):
        ...
        if canva == self.gameArea:
            ...
            if self.attacking:
                ...
                self.parent.setBuildingFlag(pos)
            else:
    ...
    def attack(self,eve):
        self.attacking = True
    #methode checker lespace
    
    #methode test build    
    def buildOn(self,eve):
        self.buildKey = True
    
    def buildOff(self,eve):
        self.buildKey = False
    ...
    def assignControls(self):
        ...
        self.gameArea.bind("A",self.attack)
        self.gameArea.bind("B",self.buildOn)
        self.gameArea.bind("b",self.buildOn)
        self.gameArea.bind("<KeyRelease-B>",self.buildOn)
        self.gameArea.bind("<KeyRelease-b>",self.buildOff)
