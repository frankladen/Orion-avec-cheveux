...
class Controller():
 ...
 #Pour changer le flag des unites selectionne pour le deplacement    
    ...
    def setBuildingFlag(self, location):
        units = ''
        #Si plusieurs unit�s sont s�lectionn�es, on les ajoute toutes dans le changement � envoyer
        for i in self.players[self.playerId].selectedObjects:
            if i.name == 'Scout':
                units+= str(self.players[self.playerId].units.index(i))+","
        print(units)
        self.pushChange(units, Flag(i,t.Target([location[0],location[1],0]),FlagState.BUILD))                       
...
#        self.players[killedIndexes[1]].units.pop(killedIndexes[0])
    ...
    def pushChange(self, playerObject, flag):
        if isinstance(flag, Flag):
            if flag.flagState == FlagState.MOVE or flag.flagState == FlagState.STANDBY:
                ...
                print('changement en poussement')
                actionString = str(self.playerId)+"/"+str(playerObject)+"/"+str(flag.flagState)+"/"+str(flag.finalTarget.position)    
    ...
    def checkVacantSpace(self, galaxy, players):
        #Psition choisie = ou pas Ã  la position du sun: direction x=0, direction y=1
        for i in galaxy.solarSystemList:
            if i.sunPosition[0] > self.position[0]-10 and i.sunPosition[0] < self.position[0]+10:
                if i.sunPosition[1] > self.position[1]-10 and i.sunPosition[1] < self.position[1]+10:
                    return False
            #Psition choisie = ou pas Ã  la position du planet: direction x=0, direction y=1
            for j in i.planets:
                if j.position[0] > self.position[0]-10 and j.position[0] < self.position[0]-10:
                    if j.position[1] > self.position[1]-10 and j.position[0] < self.position[1]+10:
                        return False
        #Psition choisie = ou pas Ã  la position du unit: direction x=0, direction y=1
        for k in players.units:
            if k.position[0] > self.position[0]-10 and k.position[1] < self.position[0]+10: 
                if k.position[1] > self.position[1]-10 and k.position[1] < self.position[1]+10:
                    return False 
        #Psition choisie = ou pas Ã  la position du building: direction x=0, direction y=1
        ...
        if action == str(FlagState.MOVE) or action == str(FlagState.STANDBY):
            ...
            for i in unitIndex:
                ...
                if i != '':
                    print(i)
                    print(action)
                    self.players[actionPlayerId].units[int(i)].changeFlag(t.Target([target[0],target[1],target[2]]),int(action))
                    #self.players[actionPlayerId].units[int(i)].changeFlag(self.players[int(target[1])].units[int(i)], int(action))
