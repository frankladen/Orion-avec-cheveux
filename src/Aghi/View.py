...
class View():              
    ...
    def fGame(self):
        ...
        #aghi images dans le canvas
        self.createActionMenu()
        #self.Actionmenu.create_image(0,0,image=self.gifDefense,anchor = NW,tag='defense')
        #self.Actionmenu.create_image(37,0,image=self.gifMove,anchor = NW,tag='move')
        #self.Actionmenu.create_image(74,0,image=self.gifDelete,anchor = NW,tag='delete')
        #self.Actionmenu.create_image(111,0,image=self.gifIcone1,anchor = NW,tag='icone1')
        self.drawWorld()
    ...
    def drawWorld(self):
        ...
        for i in players:
            for j in i.units:
                self.drawUnit(j, i)
        if self.dragging:
            self.drawSelectionBox(j,players)
        self.drawMinimap()
    ...
    def drawPlanet(self, planet, player):
        ...
        if player.camera.isInFOV(planetPosition):
            ...
            if planet in player.selectedObjects:
                ...
                #self.Actionmenu.create_image(0,34,image=self.gifIcone2,anchor = NW,tag='icone2')
    ...
    def drawSelectionBox(self,unit,player):
        selectionBoxFrame = Frame(self.root, bg="green")
    ...
    def assignControls(self):
        ...
        #aghi
        self.Actionmenu.bind("<Button-1>", self.leftclic)
