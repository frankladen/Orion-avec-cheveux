...
###################################################################################
#################################kk################################################
    def startBuild(self, x, y):
        if self.checkVacantSpace(x, y):
            
    
    def checkVacantSpace(self, x, y):
        #Position choisie = ou pas Ã  la position du sun: direction x=0, direction y=1
        for i in self.galaxy.solarSystemList:
            if i.sunPosition[0] > x-10 and i.sunPosition[0] < x+10:
                if i.sunPosition[1] > y-10 and i.sunPosition[1] < y+10:
                    return False
            #Position choisie = ou pas Ã  la position du planet: direction x=0, direction y=1
            for j in i.planets:
                if j.position[0] > x-10 and j.position[0] < x+10:
                    if j.position[1] > y-10 and j.position[0] < y+10:
                        return False
        #Position choisie = ou pas Ã  la position du unit: direction x=0, direction y=1
        for k in self.players.units:
            if k.position[0] > x-10 and k.position[1] < x+10: 
                if k.position[1] > y-10 and k.position[1] < y+10:
                    return False 
        #Position choisie = ou pas Ã  la position du building: direction x=0, direction y=1
        for m in self.players.spaceBuildings:
            if m.position[0] > x-10 and k.position[1] < x+10: 
                if m.position[1] > y-10 and k.position[1] < y+10:
                    return False
